...
import org.dbflute.saflute.core.interceptor.PublicBasisPointcut;
import org.dbflute.saflute.web.task.callback.TaskCallback;

/**
 * The default pointcut for batch task. <br>
 * Basically this is very similar to action's default pointcut. <br>
 * Actually created by copy at first but no recycle for little dependency between action and batch.
 * @author jflute
 */
public class TaskDefaultPointcut extends PublicBasisPointcut {
    ...
    static {
        ...
        final Method[] methods = TaskCallback.class.getMethods();
    ...
    protected boolean isCallbackMethodImplementation(Method method) {
        if (!TaskCallback.class.isAssignableFrom(method.getDeclaringClass())) {
