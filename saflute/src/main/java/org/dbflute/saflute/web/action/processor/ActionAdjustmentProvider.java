...
public interface ActionAdjustmentProvider {
     ...
     * @param actionMappingWrapper The wrapper of action mapping for current action. (NotNull)
     * @return The filtered path for JSP. (NullAllowed: if null, no filter)
     */
    String filterJspPath(String path, ActionMappingWrapper actionMappingWrapper);

    /**
     * Prepare the word list for retry calculation of action path for the JSP. <br>
     * You can retry calculation by filtering like this:
     * <pre>
     * e.g. /member/sp_member_list.jsp, wordList:[sp, member, list]
     *  normally /member/ but /member/list/ by filteredList:[member, list]
     * </pre>
     * @param requestPath The path of requested JSP, that doesn't have view-prefix. (NotNull)
     ...
     */
    List<String> prepareJspRetryWordList(String requestPath, List<String> wordList);
     ...
     * @return The determination, true or false. If false, default determination for routing.
     */
    boolean isForcedSuppressRedirectWithSlash(HttpServletRequest request, String requestPath,
