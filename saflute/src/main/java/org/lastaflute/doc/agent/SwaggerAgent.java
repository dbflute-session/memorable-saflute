...
public class SwaggerAgent {
    ...
    public String prepareSwaggerUiResponse(String swaggerJsonUrl) {
        return toHtmlResponse(buildSwaggerUiUrl(swaggerJsonUrl));
    }

    public String buildSwaggerUiUrl(String swaggerJsonUrl) {
        final String requestUrl = requestManager.getRequest().getContextPath() + swaggerJsonUrl;
    ...
    protected String toHtmlResponse(String swaggerUiUrl) {
        ...
        return ContainerUtil.getComponent(ActionResolver.class).toRedirectPath(swaggerUiUrl);
    }
}
