...
#                                                                                 Overview
#                                                                                 ========
SAFlute の Example プロジェクト「Maihama」のドメイン(WEBアプリ)パートです。
...
ここでは架空のプロジェクト「Maihama」を想定し、
その中の一つのドメイン(アプリケーション)「Dockside」を構成しています。

 Maihama (refers to SAFlute extends SAStruts)
  |
  |-Dockside
  |-...
  |-...

 maihama-base     : ベースプロジェクト、parent の pom や、環境設定ファイルやERDなどのドキュメントなど
 maihama-common   : 共通プロジェクト、全てのドメインの共通ライブラリ、DBFluteの自動生成クラスもある
 maihama-dockside : ドメインプロジェクト、実際のWEBアプリケーション、Actionクラスなど
 saflute          : SAFlute本体のプロジェクト、web.xmlやdiconやJavaクラスなどから参照されるクラスなど

　※safluteは別リポジトリに置いてありますので、別途cloneしてください
...
この Maihama プロジェクトをそのまま修正して「自分のプロジェクト用」にしていきます。

# ----------------------------------------------------------
#                                                Basic Setup
#                                                -----------
まずは、safluteをcloneします。SAFluteの本体のソースコードです。
そして、maihamaをcloneして、
maihama-base, maihama-common, maihama-dockside の三つのプロジェクトをimportし、
そして、maihamaなどの名前をひたすら名前を変更していきます。

  o maihama -> 自分のプロジェクト名
  o dockside -> 自分のドメイン名 (WEBアプリ名)
...
例えばクラス名は Maihama と先頭大文字になるので大文字小文字は区別しましょう。
また、MaihamaFwAssistantDirectorに、そのまま置換ではダメな値があります(見ればわかります)。
...
そのとき、パッケージ名は org.dbflute.maihama 部分をまるごとプロジェクト用に修正します。
...
とにかく、maihama や dockside という文字が検索でHITしなくなるまで修正し続けます。
